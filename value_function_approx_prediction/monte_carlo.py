...
def MC_gradient_descent():
    ...
    for ep in range(n_episodes):
        ...
        # Calcular retornos hacia atrás y actualizar w
        G = 0
        visited = set()
        for t in reversed(range(len(episode))):
            ...
            if s not in visited:  # First-visit MC
                visited.add(s)
                V_hat = weights @ phi(s)
                weights += alpha * (G - V_hat) * phi(s)
        
    V = weights.reshape((grid_size, grid_size))
    
    plt.imshow(V, cmap="viridis")
    plt.colorbar(label="V(s) estimado")
    plt.title("Linear Monte Carlo Policy Evaluation (Política Aleatoria)")
    plt.show()
...
def TD0_vfa_prediction_gradient_descent():
    grid_size = 5
    n_states = grid_size * grid_size
    actions = [(0, 1), (0, -1), (1, 0), (-1, 0)]  # derecha, izquierda, abajo, arriba
    terminal_states = [0, n_states - 1]  # esquinas (0,0) y (4,4)
